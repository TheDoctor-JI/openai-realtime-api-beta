...
export class RosInterface {
  ...
  //Event string
  ROBOT_START_SPEAKING = "rss";
  ROBOT_STOP_SPEAKING = "rse";
  USER_START_SPEAKING = "us";
  USER_STOP_SPEAKING = "ue";
  ...
  constructor() {
    ...
    /*
    Connect to ros master node via rosbridge
    */

    this.ros = new roslibjs.Ros({
    ...
    /*
    Topics for indicating conversation events
    */
    this.conv_event_topic = new roslibjs.Topic({
      ros : this.ros,
      name : '/grace_proj/dummy_behav_state',
      messageType : 'std_msgs/String'
    });

  }

  pbulish_conv_events(event_str) {
    var msg = new roslibjs.Message({
      data : event_str
    });
    this.conv_event_topic.publish(msg);
  }
...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.defaultSessionConfig = {
      ...
      instructions: '',
      voice: 'sage',
      //Female voice: "alloy"|"coral"|"sage"|"shimmer"
    ...
    this.ros_interface = new RosInterface();

    //Time the response generation
    this.response_trigger_time = 0;
    this.response_onset_time = 0;
  }
  ...
  _addAPIEventHandlers() {
    ...
    const handlerWithDispatch = (event, ...args) => {
      ...
      if(event == 'server.response.audio.delta'){
        if(this.response_onset_time == NaN){
          this.response_onset_time = new Date().getTime() / 1000;
          console.log("Response Onset Time: ", this.response_onset_time);
          console.log("Response Latency: ", this.response_onset_time - this.response_trigger_time);
        }
      }
    ...
    this.realtime.on('server.input_audio_buffer.speech_started', (event) => {
      // TBD criticial: interruption handling of this realtime console
      handler(event);
      this.dispatch('conversation.interrupted');
      this.ros_interface.pbulish_conv_events(this.ros_interface.USER_START_SPEAKING);
    });
    this.realtime.on('server.input_audio_buffer.speech_stopped', (event) => {
      handler(event, this.inputAudioBuffer);
      this.ros_interface.pbulish_conv_events(this.ros_interface.USER_STOP_SPEAKING)
    });
  ...
  createResponse() {
    ...
    //Set the trigger time, reset the onset time
    this.response_trigger_time = new Date().getTime() / 1000;
    this.response_onset_time = NaN;
    console.log("Response Trigger Time: ", response_trigger_time);
   ...
   */
  async indicateUserSpeaking() {
    this.ros_interface.pbulish_conv_events(this.ros_interface.USER_START_SPEAKING);
  }
  async indicateUserStoppedSpeaking() {
    this.ros_interface.pbulish_conv_events(this.ros_interface.USER_STOP_SPEAKING);
  }
  async indicateRobotSpeaking() {
    this.ros_interface.pbulish_conv_events(this.ros_interface.ROBOT_START_SPEAKING);
  }
  async indicateRobotStoppedSpeaking() {
    this.ros_interface.pbulish_conv_events(this.ros_interface.ROBOT_STOP_SPEAKING);
  }
